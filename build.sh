...
function build_uboot(){
	...
	(cd ${SDFUSE_DIR} && {
		...
		DISABLE_MKIMG=1 UBOOT_SRC=${TOP_DIR}/u-boot ./build-uboot.sh friendlywrt
	})
...
function build_kernel(){
	...
	(cd ${SDFUSE_DIR} && {
		...
		DISABLE_MKIMG=1 KCFG="${TARGET_KERNEL_CONFIG}" KERNEL_SRC=${TOP_DIR}/kernel ./build-kernel.sh friendlywrt
	})
